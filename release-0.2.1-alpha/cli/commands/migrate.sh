...
# Check if source directory has modules
check_source_directory() {
    ...
    local module_count
    module_count=$(find "$source_dir" -maxdepth 1 -type d -not -name "$(basename "$source_dir")" 2>/dev/null | wc -l)

    if [[ $module_count -eq 0 ]]; then
        log_warning "No modules found in source directory: $source_dir"
        return 1
    fi

    log_info "Found $module_count modules in source directory"
    return 0
}
...
migrate_main() {
    ...
    if [[ -z "$source_dir" ]]; then
        ...
        if [[ -d "./config" ]]; then
            ...
            source_dir="$(dirname "$0")/config"
        else
            log_error "No source directory specified and no default found"
            log_info "Use: $PROGRAM_NAME migrate --from <directory>"
            exit 1
        fi
    fi
    ...
    if [[ $FORCE -eq 0 ]] && ! confirm "Proceed with migration?"; then
